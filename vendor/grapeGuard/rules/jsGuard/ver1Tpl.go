...
const (
	...
	<div id="js-alert-box" class="alert-box">
		...
		<div class="alert-body">
			...
			<a id="js-alert-btn" class="alert-btn" href="/?__gKey={{.Key}}">立即进入</a>
		</div>
		<div class="alert-footer clearfix">
			<svg width="46px" height="42px" class="alert-footer-icon">
				<circle fill-rule="evenodd" clip-rule="evenodd" fill="#7B7B7B" stroke="#DEDFE0" stroke-width="2" stroke-miterlimit="5" cx="21.917" cy="21.25" r="17"/>
			...
			<div class="alert-footer-text"><p>XGuard</p>Powered By GrapeGuard</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		{
			var exdate=new Date()
			exdate.setDate(exdate.getTime()+sec*1000)
			document.cookie=c_name+ "=" +escape(value)+
			((sec==null) ? "" : ";expires="+exdate.toGMTString())
		}
	
		function alertSet(e) {
			setCookie("grapeToken","{{.Key}}",60)
			// decode
